...
export default function VolumeChart({ data }: { data: VolumeDataPoint[] }) {
  const maxSets = Math.max(
    ...data.map((d) => Math.max(d.sets, d.mav, d.plannedSets ?? 0)),
    1
  );
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
      <div className="space-y-3">
        {data.map((d) => {
          const hasPlanned = (d.plannedSets ?? 0) > 0;
          const plannedWidth = Math.min(((d.plannedSets ?? 0) / maxSets) * 100, 100);
          const actualWidth = Math.min((d.sets / maxSets) * 100, 100);
            ...
            <div key={d.muscle} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex-1 mx-3 relative h-5">
                  ...
                  {hasPlanned && (
                    <div
                      className="absolute inset-y-0 left-0 rounded-full border border-zinc-500/50 bg-zinc-700/20"
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-zinc-600">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <div className="h-3 w-5 rounded-sm border border-zinc-500/50 bg-zinc-700/20" />
            <span>Planned</span>
          </div>
        </div>
      </div>
